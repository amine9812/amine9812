<div align="center">

# Hi, I’m **Amine** 👋

**A student** building reliable systems across **Linux/Unix**, **C/C++**, and **full-stack applications** (desktop & web).

<a href="https://github.com/YOUR_GITHUB_USERNAME">
  <img src="https://komarev.com/ghpvc/?username=amine9812&style=flat-square" alt="profile views" />
</a>
<a href="https://github.com/YOUR_GITHUB_USERNAME?tab=followers">
  <img src="https://img.shields.io/github/followers/amine9812?label=Followers&style=flat-square" alt="followers" />
</a>
<a href="https://github.com/YOUR_GITHUB_USERNAME?tab=stars">
  <img src="https://img.shields.io/github/stars/amine9812?label=Stars&style=flat-square" alt="stars" />
</a>

<br/>

<img src="https://readme-typing-svg.demolab.com?font=Inter&pause=1000&center=true&vCenter=true&width=900&lines=C%2FC%2B%2B+%7C+Python+%7C+Bash+%7C+JavaScript%2FTypeScript;Linux%2FUnix+tooling+%7C+Automation+%7C+Clean+Architecture;Electron+%2B+React+Desktop+Apps+%7C+Node.js+APIs+%7C+SQL+databases;Tests+%7C+CI+%7C+Documentation+first" alt="Typing SVG" />

</div>

---

## About me

- I build **production-minded applications** with emphasis on **maintainability, security, and UX**.
- Comfortable from low-level (**C/C++**) to high-level (**Python**, **TypeScript**) and deployment tooling.
- Strong with **Linux/Unix environments**, **shell scripting**, and automation.

---

## Tech Stack

### Languages
![C](https://img.shields.io/badge/C-111827?style=for-the-badge&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C%2B%2B-111827?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![Python](https://img.shields.io/badge/Python-111827?style=for-the-badge&logo=python&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-111827?style=for-the-badge&logo=gnubash&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-111827?style=for-the-badge&logo=javascript&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-111827?style=for-the-badge&logo=typescript&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-111827?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-111827?style=for-the-badge&logo=css3&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-111827?style=for-the-badge&logo=postgresql&logoColor=white)

### Frameworks & UI
![Electron](https://img.shields.io/badge/Electron-111827?style=for-the-badge&logo=electron&logoColor=white)
![React](https://img.shields.io/badge/React-111827?style=for-the-badge&logo=react&logoColor=white)
![Next.js](https://img.shields.io/badge/Next.js-111827?style=for-the-badge&logo=nextdotjs&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-111827?style=for-the-badge&logo=tailwindcss&logoColor=white)
![Qt](https://img.shields.io/badge/Qt-111827?style=for-the-badge&logo=qt&logoColor=white)

### Backend & APIs
![Node.js](https://img.shields.io/badge/Node.js-111827?style=for-the-badge&logo=node.js&logoColor=white)
![Express](https://img.shields.io/badge/Express-111827?style=for-the-badge&logo=express&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-111827?style=for-the-badge&logo=fastapi&logoColor=white)

### Databases & ORM
![SQLite](https://img.shields.io/badge/SQLite-111827?style=for-the-badge&logo=sqlite&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-111827?style=for-the-badge&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-111827?style=for-the-badge&logo=mysql&logoColor=white)
![Prisma](https://img.shields.io/badge/Prisma-111827?style=for-the-badge&logo=prisma&logoColor=white)

### DevOps / Tooling
![Linux](https://img.shields.io/badge/Linux-111827?style=for-the-badge&logo=linux&logoColor=white)
![Git](https://img.shields.io/badge/Git-111827?style=for-the-badge&logo=git&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-111827?style=for-the-badge&logo=githubactions&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-111827?style=for-the-badge&logo=docker&logoColor=white)

### Testing
![Vitest](https://img.shields.io/badge/Vitest-111827?style=for-the-badge&logo=vitest&logoColor=white)
![Playwright](https://img.shields.io/badge/Playwright-111827?style=for-the-badge&logo=playwright&logoColor=white)
![Pytest](https://img.shields.io/badge/Pytest-111827?style=for-the-badge&logo=pytest&logoColor=white)

---

## What I work on

- **Linux/Unix scripting** (Bash, coreutils, automation)
- **C/C++** projects (OOP, performance, correctness)
- **Desktop apps** (Electron/React + local SQLite)
- **APIs & services** (Node.js/Express, Python/FastAPI)
- **Databases** (SQLite/PostgreSQL/MySQL) + schema design and migrations

---

## Featured Projects

> Pin your best repositories on your GitHub profile and list them here.

- **📌 [Erc-Sim](https://github.com/amine9812/simulator_ERC.git)** — Simulator for a mice maze hackathon w allowing to test the code before deploying it to the bot and testing the best algorithme for the maze .
- **📌 [Pixel-game](https://github.com/amine9812/our-app1.git)** — pixel art game website.
- **📌 [e-com](https://github.com/amine9812/e-com.git)** — e-comerce website.

---

## GitHub Stats

![amine9812's GitHub stats](https://github-readme-stats.vercel.app/api?username=amine9812&show_icons=true&theme=radical)


---

## Connect

- Portfolio: **LOADING**

---

## Quick Notes

- **Environment:** Linux / Unix
- **Focus:** clean architecture, testing, documentation, and performance
- **Open to:** internships, freelance projects, collaborations

<div align="center">

_“Build systems that are simple to run, hard to break, and easy to extend.”_

</div>
